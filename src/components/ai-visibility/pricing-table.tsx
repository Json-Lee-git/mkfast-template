        ...
        <tbody className="divide-y divide-zinc-800/50">
          {tools.map((tool) => (
            <tr key={tool.slug} className="hover:bg-zinc-900/30">
              ...
              <td className="px-3 py-2">{tool.hasFreePlan ? <span className="text-emerald-400">✓</span> : <span className="text-zinc-600">—</span>}</td>
              <td className="px-3 py-2">{tool.hasFreeChecker ? <span className="text-blue-400">Free checker</span> : <span className="text-zinc-600">—</span>}</td>
              <td className="px-3 py-2 text-zinc-400 text-xs">{tool.bestFor.slice(0, 2).join(", ")}</td>
              <td className="px-3 py-2 text-xs text-zinc-500 max-w-48 truncate">{tool.pricingNotes}</td>
            </tr>
          ))}
        </tbody>
      </table>
