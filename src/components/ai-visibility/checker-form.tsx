...
export function CheckerForm({ onSubmit }: { onSubmit: (data: CheckerFormData) => void }) {
  ...
  const handleSubmit = (e: FormEvent) => { e.preventDefault(); onSubmit(form); setSubmitted(true); };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <button type="submit" className="w-full rounded-md bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-500 transition-colors dark:bg-blue-600 dark:hover:bg-blue-500">Submit & Get Sample Report</button>
    </form>
  );
}
