...
function ComparePage() {
  ...
  if (!comp) {
    return (
      <Container className="py-16 text-center">
        <h1 className="text-2xl font-bold text-zinc-100">Comparison not found</h1>
  ...
  if (!toolA || !toolB) {
    return (
      <Container className="py-16 text-center">
        <h1 className="text-2xl font-bold text-zinc-100">Tool data not available</h1>
      </Container>
    );
  }
    ...
    <div>
      <section className="border-b border-zinc-800 py-16">
        <Container>
          ...
          <h1 className="mt-4 text-3xl font-bold text-zinc-100">{comp.title}</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800">
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-emerald-400">{toolB.name}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800/50">
                {[
                  ...
                  <tr key={label}>
                    ...
                    <td className="px-4 py-2 text-zinc-300">{a}</td>
                    <td className="px-4 py-2 text-zinc-300">{b}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="border-t border-zinc-800 py-8">
        <Container>
          <h2 className="text-xl font-bold text-zinc-100">Pricing Comparison</h2>
      ...
      <section className="border-t border-zinc-800 py-8">
        <Container>
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-emerald-400">Best for {toolB.name}</h2>
              <ul className="mt-4 space-y-2">
      ...
      <section className="border-t border-zinc-800 py-8">
        <Container>
          ...
          <p className="mt-4 text-zinc-300">{comp.recommendationDetail}</p>
        </Container>
      </section>
      ...
      {related.length > 0 && (
        <section className="border-t border-zinc-800 py-8">
          <Container>
            <h2 className="text-xl font-bold text-zinc-100">Other Comparisons</h2>
            <div className="mt-4 space-y-2">
              {related.map((r) => (
                <a key={r.slug} href={`/compare/${r.slug}`} className="block text-blue-400 hover:text-blue-300">→ {r.title}</a>
              ))}
            </div>
          </Container>
        </section>
