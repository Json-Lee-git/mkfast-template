...
function buildFreeReportMarkdown(result: AeoAuditResult): string {
  ...
  const lines = [
    ...
    '',
    '## Diagnostic Evidence: AI Files Audit',
    '',
...
function getFixPackPreview(result: AeoAuditResult): string {
  if (result.aiAnalysis?.actionPlan.length) {
    const top = result.aiAnalysis.actionPlan[0];
    return `${top.title} — ${top.whatToDo.slice(0, 90)}`;
  }
  if (result.recommendations.length > 0) {
    return result.recommendations[0];
  }
  return 'Copy-ready fixes for your specific page issues';
}
...
const faqItems = [
  {
    q: 'What is an AEO checker?',
    a: 'An AEO checker is a tool that audits whether a page is ready for answer engines and AI search systems. This free AEO checker reviews crawl access, schema, answer-ready content, entity clarity, trust signals, and AI-readable files.',
  },
  {
    q: 'What does this AEO checking tool analyze?',
    a: 'This AEO checking tool analyzes crawlability, structured data, LLMs.txt files, AI crawler access, content structure, entity clarity, trust signals, and whether the page has concise answer blocks that AI systems can extract.',
  },
  {
    q: 'Does this tool measure AI search visibility?',
    a: 'It measures AI search visibility readiness, not live visibility. The score shows whether your page has the technical and content signals that help AI systems understand it. It does not track rankings, citations, or live answer inclusion.',
  },
  {
    q: 'Does this help with Google AI Overviews?',
    a: 'It helps you check readiness signals that can matter for AI-assisted search surfaces, including crawlability, schema, content clarity, and trust. It does not guarantee Google AI Overview inclusion, rankings, traffic, or citations.',
  },
  {
    q: 'Can this check ChatGPT search readiness?',
    a: 'Yes. The audit checks public-page readiness signals that can affect whether ChatGPT-style search systems can crawl, parse, understand, and trust a page. It is not a ChatGPT ranking tracker.',
  },
  {
...
const aeoAnalysisItems = [
  {
    title: 'Crawl and indexability',
    desc: 'HTTP status, canonical URL, meta robots, sitemap signals, and whether the main page content is reachable.',
  },
  {
    title: 'Answer-ready content',
    desc: 'H1 and H2 structure, question headings, short answer paragraphs, FAQ coverage, and extractable page summaries.',
  },
  {
    title: 'Structured data',
    desc: 'JSON-LD schema such as Organization, WebSite, Article, FAQPage, Product, SoftwareApplication, and BreadcrumbList.',
  },
  {
    title: 'AI crawler access',
    desc: 'Robots.txt policy for AI-related crawlers and search assistants, including common OpenAI, Anthropic, Perplexity, and Google agents.',
  },
  {
  ...
  },
  {
    ...
    desc: 'Publisher clarity, brand/entity cues, author or review signals, dates, contact paths, and page-level trust context.',
  },
];
...
function AeoCheckerPage() {
  ...
  const [auditSource] = useState(() => {
    if (typeof window === 'undefined') return '';
    return new URLSearchParams(window.location.search).get('source') ?? '';
  });
  const [industryContext] = useState(() => {
    if (typeof window === 'undefined') return '';
    return new URLSearchParams(window.location.search).get('industry') ?? '';
  });
  const [competitorContext] = useState(() => {
    if (typeof window === 'undefined') return '';
    return new URLSearchParams(window.location.search).get('competitors') ?? '';
  });
  ...
  const competitorList = competitorContext
    .split(',')
    .map((competitor) => competitor.trim())
    .filter(Boolean);
  const hasVisibilityContext =
    auditSource === 'chatgpt-visibility-checker' ||
    Boolean(industryContext) ||
    competitorList.length > 0;

  useEffect(() => {
    trackConversionEvent('aeo_page_viewed');
  }, []);

  useEffect(() => {
    if (result && !previewFiredRef.current) {
      previewFiredRef.current = true;
      trackConversionEvent('locked_fix_preview_seen', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackConversionEvent('scan_failed', { url: trimmed, reason });
      setError(
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        capturedEmailRef.current = leadEmail.trim();
  ...
  const handleCheckout = async () => {
    ...
    const email = capturedEmailRef.current || undefined;
    try {
        ...
        <Container className="px-4">
          <div className="grid gap-7 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
            <div className="max-w-3xl">
              ...
              <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm leading-6 text-muted-foreground">
                <p>
                  AEO Checker is the page-level diagnosis layer inside the
                  broader AI Visibility Audit workflow. If you want to start
                  with the commercial question first, check whether ChatGPT-like
                  answers can understand and recommend your brand.
                </p>
                <a
                ...
                >
                  Start with ChatGPT Visibility Checker
            ...
            <div className="flex gap-8">
              ...
              <div className="flex-1 min-w-0 space-y-8">
                            ...
                            value:
                              r.score >= 80
                                ...
                                : r.score >= 60
                                  ...
                                  : 'Blocked',
                            detail:
                              'Signals that help AI systems understand the page offer.',
                          },
                          {
                            ...
                            value:
                              ...
                              r.trustSignals.hasAuthor
                                ...
                                : 'Needs work',
                            detail:
                              'Answer blocks and trust signals that support extraction.',
                          },
                          {
                            title: 'Fix priority',
                            value:
                              r.recommendations.length > 0
                                ? `${r.recommendations.length} blockers`
                                : 'No major blockers',
                            detail:
                              'Visibility blockers ordered by expected repair impact.',
                          },
                        ].map((item) => (
                          <div
                            key={item.title}
                    ...
                    <DashboardCard>
                      ...
                      {competitorList.length > 0 ? (
                        ...
                        <p className="mt-3 text-sm text-muted-foreground">
                          Add competitors in the ChatGPT Visibility Checker to
                          frame the next audit.
                        </p>
                      )}
                      <p className="mt-4 text-sm leading-6 text-muted-foreground">
                        Competitors help frame the prompts and query gaps for
                        future audits. This version does not run live competitor
                        ranking checks.
                      </p>
                    </DashboardCard>
                  </section>
                ...
                <section id="audit-crawl">
                  <DashboardCard>
                    <DashboardSectionHeader
                      title="Diagnostic breakdown: crawlability"
                      status="good"
                    />
                ...
                <section id="audit-fixes">
                  <DashboardCard>
                    ...
                    <p className="mb-4 text-sm text-muted-foreground">
                      Visibility blockers, ordered by expected impact on AI
                ...
                <div className="rounded-2xl border border-primary/20 bg-primary/5 p-8 text-center">
                  ...
                  <div className="mt-6 rounded-xl border border-primary/20 bg-background/70 p-4">
        ...
        <Container>
          <div className="mx-auto max-w-3xl">
            <h2 className="text-center text-2xl font-bold text-foreground">
              What does this AEO checker analyze?
            </h2>
            <p className="mt-6 text-center text-muted-foreground">
              This free AEO checker analyzes the technical and content signals
              that can make a page easier for answer engines and AI search
              systems to understand. It is built for page-level AEO checking,
              not live ranking or citation tracking.
            </p>
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {aeoAnalysisItems.map((item) => (
                <div
                  key={item.title}
...
export const Route = createFileRoute('/tools/aeo-checker')({
  head: () => ({
    ...seo('/tools/aeo-checker', {
      title: 'Free AEO Checker - AI Search Visibility & Answer Engine Audit',
      description:
        'Run a free AEO checker for AI search visibility readiness. Audit crawl access, schema, LLMs.txt, answer blocks, Google AI Overviews signals, and ChatGPT search readiness.',
    }),
    scripts: [
      ...
      jsonLd(
        softwareApplicationSchema({
          ...
          keyFeatures: [
            ...
            'Fix-this-first repair recommendations',
